// Paso 5 — Revisión y publicación (M5.5): resumen «por excepciones» (solo lo que
// falta), visibilidad y unidades. El guardado usa handleSave del hook (byte-
// idéntico); la visibilidad y las unidades se delegan a TeacherDash mediante los
// callbacks opcionales. Diseño según docs/especificacion_editor.html (stepRevision).
...
interface Falta { txt: string; go: number }

export function Paso5Revision({
  ed, goStep, units = [], onToggleVisibility, onAddToUnit, onRemoveFromUnit,
  ...
  return (
    <>
      <StepHead n={5} title="Revisión y publicación" desc="Comprueba lo que falta, hazlo visible y asígnalo a unidades." />
            ...
            <>
              ...
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                {faltas.map((it, i) => (
                  <li key={i}>
                    <button type="button"
                      onClick={() => (it.go === 5 && onToggleVisibility ? onToggleVisibility() : goStep(it.go))}
                      ...
                      <span style={{ fontSize: 11.5, color: C.quiz, fontWeight: 600, whiteSpace: "nowrap" }}>
                        {it.go === 5 && onToggleVisibility ? "Hacer visible" : `Ir al paso ${it.go} →`}
                      </span>
                    </button>
                  </li>
                ))}
