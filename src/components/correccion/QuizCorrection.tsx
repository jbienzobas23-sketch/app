  ...
  const releaseFragment = () => {
    ...
    if (isTeacherMode) {
      ...
      const veredictoDe = (q: (typeof questions)[number]): Veredicto => {
        if (q.type === "desarrollo") return { symbol: "✎", word: "A tu corrección", color: C.quiz, bg: "rgba(47,111,184,0.08)" };
      ...
      // Etiqueta pequeña «alumno» sobre la opción/respuesta elegida: es la ÚNICA
      // marca de autoría — la corrección la dan el ✓/✗ y el color de la fila.
            ...
            <div style={gridStyle}>
              ...
              <aside style={asideStyle} className="fa-autohide-scroll" onScroll={handleAutoHideScroll}>
                ...
                {/* Índice de preguntas: veredicto + número + inicio del texto.
                    ...
                    (regla 9 del proyecto, daltonismo). */}
        ...
        <div style={S.page}>
          ...
          <div style={{ ...S.card, textAlign: "center", marginBottom: 20 }}>
            {tc?.corrected && tc?.totalScore != null ? (() => {
              ...
              return (
                <>
                  <div style={{ fontSize: 64, fontWeight: 900, color: scoreColor(pct100), lineHeight: 1 }}>{nota10(pct100)}</div>
                  <div style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>Nota final · corregido por el profesor</div>
                  {sc != null && <div style={{ color: C.muted, fontSize: 12, marginTop: 8 }}>{correctN} de {gradableQs} {gradableQs === 1 ? "pregunta autocorregible" : "preguntas autocorregibles"} (automática: {nota10(sc)})</div>}
                </>
              );
            })() : sc != null ? (
              <>
                <div style={{ fontSize: 64, fontWeight: 900, color: col, lineHeight: 1 }}>{nota10(sc)}</div>
                <div style={{ color: C.muted, fontSize: 14, marginTop: 4 }}>
                  Nota automática · {correctN} de {gradableQs} {gradableQs === 1 ? "pregunta autocorregible" : "preguntas autocorregibles"}
                  {devQs.length > 0 && ` · ${devQs.length} de desarrollo pendiente${devQs.length === 1 ? "" : "s"} de corrección`}
                </div>
          ...
          {/* Misma rejilla de dos columnas que el modo profesor (Jon, 2026-07-05). */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(400px, 1fr))", gap: 14, alignItems: "start", marginBottom: 12 }}>
          {questions.map((q, idx) => {
            const studentAnswer = result.answers?.[q.id];
            ...
            return (
              <div key={q.id} style={{ ...S.card, marginBottom: 0, border: q.type === "desarrollo" ? `1px solid ${C.line}` : `1.5px solid ${isCorrect ? C.fnT : isWrong ? C.danger : C.line}` }}>
                {/* Chips reducidos (Jon, 2026-07-05): sin el chip de tipo — el
                    contenido de la tarjeta ya lo dice. */}
